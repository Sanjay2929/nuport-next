"use client";
import React, { useEffect, useRef, useState } from "react";
import { GrayLeftArrow, GrayRightArrow } from "../common/Icons"; // Importing arrow icons
import Slider from "react-slick"; // Importing Slider component
import { AchieveBusinessGoalsCards } from "../common/Helper"; // Importing helper function

// AchieveBusinessGoals component definition
const AchieveBusinessGoals = () => {
  const [leftMargin, setLeftMargin] = useState(0); // State for left margin

  const containerRef = useRef(null); // Ref for container

  // Effect to calculate left margin
  useEffect(() => {
    if (containerRef.current) {
      setLeftMargin(containerRef.current.getBoundingClientRect().left);
    }
    window.addEventListener("resize", () => {
      if (containerRef.current) {
        setLeftMargin(containerRef.current.getBoundingClientRect().left);
      }
    });
  }, [leftMargin]);

  const slider = useRef(null); // Ref for Slider component

  // Slider settings
  var settings = {
    slidesToShow: 3.25,
    centerPadding: "100px",
    arrows: false,
    slidesToScroll: 1,
    centerMode: true,
    dots: false,
    infinite: true,
    responsive: [
      {
        breakpoint: 1400,
        settings: {
          slidesToShow: 2.2,
          centerPadding: "100px",
        },
      },
      {
        breakpoint: 1280,
        settings: {
          slidesToShow: 2.2,
          centerPadding: "90px",
        },
      },
      {
        breakpoint: 1023.98,
        settings: {
          slidesToShow: 1.2,
          centerPadding: "110px",
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: 1.2,
          centerPadding: "85px",
        },
      },
      {
        breakpoint: 640,
        settings: "unslick",
      },
    ],
  };

  return (
    <div className="bg-darkBlue py-16 sm:py-20 md:py-28 lg:py-32 xl:py-40 overflow-hidden">
      {/* Container for content */}
      <div
        className="xl:max-w-[1324px] container px-5 mx-auto flex items-end justify-between"
        ref={containerRef} // Assigning ref to container
      >
        {/* Title and description */}
        <div data-aos="fade-up">
          <p className="text-orangeDark text-base mb-3 tracking-[2.5px] font-bold uppercase">
            How we stand out
          </p>
          <h2 className="max-w-[731px] mr-auto text-[32px] !leading-[110%] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-plus font-extrabold text-white">
            Achieve your <span className="text-garyMedium">business goals</span>
          </h2>
        </div>
        {/* Arrow icons */}
        <div className="sm:flex items-center gap-5 hidden">
          <span
            className="cursor-pointer achieve_arrow"
            onClick={() => slider?.current?.slickPrev()} // Function to go to previous slide
          >
            <GrayLeftArrow strokeColor={"#6A758F"} />
          </span>
          <span
            className="cursor-pointer achieve_arrow"
            onClick={() => slider?.current?.slickNext()} // Function to go to next slide
          >
            <GrayRightArrow strokeColor={"#6A758F"} />
          </span>
        </div>
      </div>
      {/* Slider */}
      <div
        className="mt-7 sm:mt-8 md:mt-10 lg:mt-14 xl:mt-[84px] overflow-hidden"
        style={{ marginLeft: `${leftMargin + 16}px` }} // Applying left margin
      >
        {/* Slider component */}
        <Slider ref={slider} {...settings}>
          {/* Mapping over AchieveBusinessGoalsCards */}
          {AchieveBusinessGoalsCards.map((value, index) => (
            <div key={index} className="pl-1 pr-5 sm:px-3 mt-6 sm:mt-0">
              {/* Individual card */}
              <div className="border-[1.5px] border-darkGray rounded-[20px] px-7 py-8 sm:p-7 md:pb-[37.6px] sm:h-[350px] md:h-[400px] lg:h-[400px] xl:h-[400px] 2xl:h-[440px] flex flex-col justify-between hover:border-white duration-300">
                <span className="max-w-[40px] md:max-w-[56px]">
                  {value.icon} {/* Icon */}
                </span>
                <div className="pt-[103px] sm:pt-0">
                  {/* Title */}
                  <p className="text-2xl lg:text-3xl font-bold !leading-[140%] md:!leading-[36px] lg:leading-[38px] text-white max-w-[298px]">
                    <span className="text-moonstone">{value.range}</span>{" "}
                    {value.title}
                  </p>
                  {/* Description */}
                  <p className="!leading-[140%] text-sm md:text-base lg:text-lg font-medium text-lightGray mt-3 md:mt-4">
                    {value.description}
                  </p>
                </div>
              </div>
            </div>
          ))}
        </Slider>
      </div>
    </div>
  );
};

export default AchieveBusinessGoals; // Exporting AchieveBusinessGoals component
